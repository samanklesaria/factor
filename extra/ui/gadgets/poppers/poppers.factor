...
ui.gadgets.editors ui.gadgets.layout ui.gadgets.layout-protocol
ui.gadgets.line-support ui.gadgets.tracks ui.gestures ;
FROM: models => change-model ;
FROM: ui.gadgets.layout-protocol => add-gadget unparent clear-gadget ;
...
TUPLE: popper < track submodels
    ...
    { delete-hook initial: [ drop ] } ;
M: popper output-model submodels>> ;
...
: (delete-popped) ( popped -- ) {
        [ [ control-value ] [ parent>> delete-hook>> ] bi call( a -- ) ]
        [ model>> ] [ parent>> submodels>> [ model-delete ] 2keep model-changed ]
        [ focus-prev ] [ unparent ]
    } cleave ;
...
M: popper model-changed 2dup model>> =
    ...
    ] [ [ value>> ] dip model>> (>>value) ] if ;

M: popper graft* submodels>> activate-model ;
M: popper ungraft*
    [ submodels>> [ value>> ] keep deactivate-model ]
    [ model>> ] bi (>>value) ;
